...
### M5: 🌟 AIヘルスコーチ（目玉・デモのクライマックス）
- [ ] `/api/coach/weekly`：直近7日集計 → Bedrockで週次アドバイス生成
- [ ] コーチ画面UI（達成点 / 不足栄養 / 明日への提案を表示）
- [ ] **デモ用シードデータ**（過去1〜2週間分の食事履歴）を投入し、コーチが豊かに語れる状態を用意 ← 審査対策の要

### M5.5: 買い物リスト（不足食材）※P2 → 正式実装に格上げ
- [ ] 献立の食材を「冷蔵庫にある / 追加で必要」に分離（analyze-fridge プロンプト + 型）
- [ ] 不足食材を「買い物リスト」として表示（現状の(追加推奨)ゆらぎを構造化して解消）

### M5.6: 多言語対応（日英切替 i18n）※H0グローバル審査への配慮
- [ ] i18n 基盤の導入（辞書 + 言語切替トグル）
- [ ] 既存画面（献立/ログイン/プロフィール/ダッシュボード/コーチ）を日英対応
- [ ] 提出物（デモ動画のナレーション/字幕・README）も英語化
> 順序メモ: i18n は基盤のため、M5コーチUIを作る前に入れると以降の画面追加が楽。
> ただし目玉のM5コーチを先に固める案もあり、着手順は要相談。
